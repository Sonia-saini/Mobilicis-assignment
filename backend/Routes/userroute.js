...
userRouter.get("/usersbymail", async (req, res) => {
  try {
    let users = await Usermodel.find({
      $and: [{ last_name: { $regex: /^M/i } }],
    });
    let x = users.filter(
      (el) =>
        el.quote.length > 15 && el.email.includes(el.last_name.toLowerCase())
    );
    res.status(200).json({ x });
  } catch (err) {
    ...
    const cityData = users.reduce((acc, user) => {
      ...
      if (cityIndex !== -1) {
        ...
        acc.push({
          name: user.city,
          count: 1,
          totalIncome: parseInt(user.income.split("$")[1]),
        });
      }
      return acc;
    }, []);

    const sortedCityData = cityData
      .sort((a, b) => b.count - a.count)
      .slice(0, 10);
    const averageIncome = sortedCityData.map((city) => ({
      name: city.name,
      averageIncome: city.totalIncome / city.count,
    }));
